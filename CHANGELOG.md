...
- **Upstream PR #193 merged** into `AlexandrErohin/TP-Link-Archer-C6U:main` - the AX72 login fix (`TplinkRouterAX72`, `test_client_ax72.py`, provider registration) originally built in this project is now part of upstream `tplinkrouterc6u`. The maintainer made follow-up edits to the merged code and asked for a fresh confirmation of `TplinkRouterProvider.get_client()`/`authorize()`/`get_status()` against real hardware before this project bumps its pinned dependency or removes the local `tplink_ax72.py` copy.
