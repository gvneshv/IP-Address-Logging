...
| **🚀 Phase 5: Deployment & Startup** | 🔄 | [ ✓ ] Fix Windows Task Scheduler startup task (working-directory bug, logon-trigger scope)</br>[&emsp;] Verify startup task end-to-end on a real machine</br>[&emsp;] Open upstream PR to `tplinkrouterc6u` for the AX72 fix</br> | Windows-only for now; cross-platform (e.g. systemd/cron) scheduling is out of scope unless requested |